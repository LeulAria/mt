...
import { styled, Paper, ListItemText, Box, Typography } from '@material-ui/core';
...
const Chatbox = ({ uid_1 }: any): JSX.Element => {
            ...
            .subscribe((data: any[]) => {
                const filtered = data.filter((item) => {
                    return (item.user_uid_1 === uid || item.user_uid_2 === uid) &&
                        ...
                        (item.to === UserRole.SALES_PERSON || item.to === UserRole.USER)
                })
                dispatch(clearConverations());
    ...
    const handleSendMessage = (e: any): void => {
        ...
        if (queryName) {
            ...
            dispatch(sendRealTimeUserMessage(messageContent, UserRole.SALES_PERSON))
            setQueryName("");
        }
    }
